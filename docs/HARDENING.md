...
- [x] **Media mapping (P1.5) — shim side.** Inbound `image`/`audio`/`document` → a `media:[{type,url,mime,filename}]` forward where `url` is `{SHIM_MEDIA_BASE_URL}/media/<token>`; the agent GETs it (bearer-gated) and the shim proxy-streams the file from GOWA's `statics/media/` store. Token is a stateless HMAC over the GOWA-relative path (`media.rs`) — no server state, restart-safe, traversal-proof. Outbound `POST /send/{image,audio,file}` (a `media_url` GOWA fetches, or `media_base64` uploaded as multipart; `voice`/`ptt` for voice notes), bearer + rate-limited. Contract in `AGENT-WHATSAPP-CONTRACT.md`. **Still open:** (1) the **agent-side** implementation (parse `media`, fetch + vision/Whisper, media send) — drafted in the contract, tracked separately; (2) **auto-download-off** (CDN-URL) inbound — would materialise the file via GOWA's `GET /message/:id/download?phone=` first (default is auto-download on, so out of scope); (3) **video + sticker** — a small additive extension of the same parse/route pattern.
